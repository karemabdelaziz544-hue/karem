...
// 🧹 دالة تنظيف صارمة بتمسح أي مسافات مخفية أو حروف غير مرئية من المفتاح
const cleanKey = (key: string) => key.replace(/[^a-zA-Z0-9_-]/g, "");
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // تطبيق التنظيف الصارم على المفتاح
    groqApiKey = cleanKey(groqApiKey);
        ...
        messages: [
          {
            ...
            content: [
              {
                type: "text",
                text: 'Look at this InBody sheet carefully. Identify ACTUAL values for Weight, Muscle, and Fat. Return ONLY valid JSON: { "weight": 0, "muscle": 0, "fat": 0, "summary": "..." }',
              },
              { 
                type: "image_url", 
                image_url: { 
