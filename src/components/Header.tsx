...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      <nav className={`fixed w-full z-[120] transition-all duration-300 ${scrolled || isOpen || isBlogPostPage ? 'bg-white/95 backdrop-blur-md shadow-md py-3' : 'bg-transparent py-5'
        }`} dir="rtl">
          ...
          <div className="hidden md:flex flex-grow justify-center items-center gap-6 lg:gap-8 px-4">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`font-bold transition-colors whitespace-nowrap text-sm lg:text-base ${location.pathname === link.path
      ...
      <div className={`fixed inset-0 bg-forest text-white z-[125] flex flex-col items-center justify-center gap-6 transition-transform duration-500 md:hidden ${isOpen ? 'translate-y-0' : '-translate-y-full'}`}>
        {navLinks.map((link) => (
          <Link
            ...
            className={`text-3xl font-black transition-colors ${location.pathname === link.path ? 'text-orange' : 'hover:text-orange'
              }`}
          >
            {link.name}
          </Link>
