...
import { 
  ...
  Quote, Layout, Menu, X // 👈 أضفنا Menu و X
} from 'lucide-react';
...
const AdminLayout: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden" dir="rtl">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto scrollbar-hide">
            {navItems.map(item => (
              <NavLink 
                key={item.path} 
                to={item.path}
                onClick={() => setIsMobileMenuOpen(false)} // يقفل القائمة لما تختار قسم
                end={item.path === '/admin'}
                className={({ isActive }) => `
      ...
      {/* 2. طبقة تعتيم (Overlay) عند فتح القائمة في الموبايل */}
      {isMobileMenuOpen && (
        <div 
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-gray-50 h-full">
        ...
        <header className="bg-white border-b border-gray-200 px-4 md:px-6 py-4 flex justify-between items-center shadow-sm z-40 h-[73px] shrink-0">
            {/* زرار الهمبرجر للموبايل */}
            <div className="flex items-center gap-4">
<button 
  onClick={(e) => {
    e.stopPropagation(); // يمنع تداخل الأحداث
